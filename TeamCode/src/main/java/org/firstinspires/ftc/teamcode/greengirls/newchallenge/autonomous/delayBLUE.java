 ...
 */
@Autonomous(name="delayBLUE",group="Auto")
public class delayBLUE extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            while (!finished) {
                ...
                reached = false;
                while (!reached) {
                    if (Math.abs(leftBackMotor.getCurrentPosition()) > 3500) {
                        reached = true;
                        setRightMotors(0);
                        setLeftMotors(0);
                    }
                ...
                reached = false;
                while (!reached) {
                    if (gyro.getHeading() >= 77 && gyro.getHeading() <= 300) {
                        reached = true;
                        setRightMotors(0);
                        setLeftMotors(0);
                    }
                ...
                while (!reached) {
                    telemetry.addData("heading", gyro.getHeading());
                    telemetry.update();
                    if (gyro.getHeading() < 77 || gyro.getHeading() > 77) {
                        leftSpeed = .35+((77-gyro.getHeading())*.030);
                        rightSpeed = .35-((77-gyro.getHeading())*.030);
